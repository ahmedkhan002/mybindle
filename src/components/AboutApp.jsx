...
const AboutApp = () => {
  ...
  return (
    <>
      <section id='about' className='mt-30 max-sm:mt-20 lg-h-[550px] max-sm:mx-8 mx-10'>
        <div className='flex flex-row max-lg:flex-col w-full max-lg:gap-40'>
          <div className='w-full flex justify-center items-center'>
            <div
              className={`lg:w-100 lg:h-100 max-sm:h-72 max-sm:w-72 max-lg:h-80 max-lg:w-80 top-10 relative rounded-full bg-[#FF5349]`}>
              ...
              <div className='absolute bottom-30 left-20 h-60 w-60 sm:h-80 sm:w-80 md:h-80 md:w-80 lg:h-100 lg:w-100 '>
                <motion.img
                  ...
                  transition={{ duration: 0.6, ease: "easeOut" }}
                  viewport={{ once: true, amount: 0.2 }}
                  className='relative top-5' src={rotateright} alt="" />
              </div>
            </div>
          </div>
          <div className='w-full'>
            <div className='flex flex-col gap-10 max-sm:px-0 px-5'>
              <motion.h1
                ...
                viewport={{ once: true, amount: 0.2 }}
                className='max-sm:text-4xl max-md:text-5xl text-6xl font-bold max-sm:text-center' >Where Every Click Sparks a Connection!</motion.h1>
              <motion.p
                initial={{ opacity: 0 }}
                whileInView={{ opacity: 1 }}
                transition={{ delay: '.3', duration: '1' }}
                viewport={{ once: false, amount: 0.5 }}
                className='text-lg max-sm:text-sm font-semibold max-sm:text-center'>A small act of kindness today can create a lifetime of
